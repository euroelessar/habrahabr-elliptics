...
if __name__ == '__main__':
    ...
    parser.add_argument('--user', dest='user', action='store', default=None, help='File to send with POST request')
    parser.add_argument('--token', dest='token', action='store', default=None, help='File to send with POST request')
    args = parser.parse_args()

    headers = {}
    if args.token:
        headers['Authorization'] = generate_signature(args.token, 'POST' if args.file else 'GET', args.url)
